...
void record_send(unsigned char* session_id, const void *buf, int len) {
    print_session_id(session_id);
    printf("Send <%s>\n", buf);
}

void SSL_unsafe_write(SSL_CTX* ctx, const void *buf, int len) {
    record_send(ctx->session_id, buf, len);
}
//---
...
int main()
{
    ...
    SSL_CTX* ctx = SSL_CTX_new(METHOD_SSL_V3);
    printf("\t[2] Connect\n");
    SSL_connect(ctx);
    printf("\t[3] Write\n");
